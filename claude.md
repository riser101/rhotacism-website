...
immediately.** Pushing feature work directly to `main` is allowed (owner's call,
Sep 2026 — staging-first is no longer required). A `staging` branch still exists
if a pre-prod check is ever wanted:
`https://rhotacism-website-git-staging-yousuf-syeds-projects.vercel.app` (that
alias is the only Vercel origin whitelisted for Google sign-in — don't rename
the branch).
